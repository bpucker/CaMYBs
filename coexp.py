...
__usage__ = """
					...
					--c <MIN_CORRELATION_CUTOFF>
					--p <MAX_PVALUE_CUTOFF>
					--e <MIN_EXPRESSION_CUTOFF>
...
def compare_candidates_against_all( candidate, gene_expression, cor_cutoff, pvalue_cutoff, exp_cutoff ):
	...
	for i, gene2 in enumerate( gene_expression.keys() ):
		if candidate != gene2:
			...
			if not math.isnan( r ) and total_expression > exp_cutoff:
				if r > cor_cutoff and p < pvalue_cutoff:
...
def main( arguments ):
	...
	if '--c' in arguments:
		try:
			cor_cutoff = float( arguments[ arguments.index( '--c' )+1 ] )
		except:
			cor_cutoff = 0.7
	else:
		cor_cutoff = 0.7
	...
	if '--p' in arguments:
		try:
			pvalue_cutoff = float( arguments[ arguments.index( '--p' )+1 ] )
		except:
			pvalue_cutoff = 0.05
	else:
		pvalue_cutoff = 0.05
	...
	if '--e' in arguments:
		try:
			exp_cutoff = float( arguments[ arguments.index( '--e' )+1 ] )
		except:
			exp_cutoff = 10.0
	else:
		exp_cutoff = 10.0
	...
	for candidate in high_impact_candidates:
		coexpressed_genes = sorted( compare_candidates_against_all( candidate, gene_expression, cor_cutoff, pvalue_cutoff, exp_cutoff ), key=itemgetter( 'correlation' ) )[::-1]
